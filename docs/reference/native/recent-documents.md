...
The Rust host adapter adds, clears, and lists recent documents on macOS through
`NSDocumentController`, on Windows through `SHAddToRecentDocs`, the Recent
Items known folder, and Shell Link resolution, and on Linux through GTK
`RecentManager`.
...
| `list`  | `RecentDocumentsListResult` | macOS, Windows, Linux supported |
...
`add` and `cleared` after a successful `clear`.
...
`InvalidArgument`. Host transport failure returns `HostUnavailable`.
