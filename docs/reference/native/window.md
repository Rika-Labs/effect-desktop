...
Window lookup is backed by host-routed native methods. `getCurrent` returns the focused tracked window, `getById` returns a tracked open window by id, and `list` returns tracked open windows in host creation order. The runtime validates host lookup results against its live `ResourceRegistry` handles, so a destroyed window is removed from lookup before `Window.close` completes. The internal router still emits opened, focused, and terminal closed registry events for host/runtime observers; a renderer-facing `Window.events()` stream remains reserved until the bridge event publisher is wired end to end.
