...
`flush` is fully supported for host-owned breadcrumb artifacts. The remaining
native surface methods report `partial` on macOS, Windows, and Linux because host
