...
| `native`      | `DesktopNativeLayer`           | A single composed Layer of native selections. Build via `Desktop.native(Native.all)` or selected `Native.*` layers.                     |
...
Native availability is not permission approval. Pair privileged calls with explicit native permission capabilities.
