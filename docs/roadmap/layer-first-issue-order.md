...
| #1286 Remove native generated-client casts over Effect RPC             | Removes type-only `as unknown as` adapters around Effect RPC generated clients.   | Keeps native client derivation owned by the same Effect RPC surfaces that own the contracts.                              |
...
| #1289 Make DesktopRpc.supportedGroup type-preserving                   | Removes the remaining support-filter cast or manual supported-RPC list tradeoff.  | Lets Window and later native clients derive supported callable methods from one RpcSupport metadata source of truth.      |
