...
Tracks `docs/SPEC.md` §24.23 and GitHub issue #96. Format follows the
...
- `docs/verification-matrix.json`, `tests/spec/verification-matrix.test.ts`,
  `.github/workflows/ci.yml`, and `docs/manual-gates/{macos,windows,linux}.md`
...
- `docs/verification-matrix.json` is the checked release-matrix data artifact
...
as manual gates under `docs/manual-gates/`.
...
pending release sign-off in `docs/manual-gates/`.
...
- #369 added `docs/verification-matrix.json`, matrix tests, Blacksmith CI cell
