...
| 5   | Preserve execute bits when staging replacement bundle                                                                    | Address | Unix replacements now preserve existing current-bundle permissions on the staged file and destination-local commit temp; added an execute-bit regression test.           |
| 6   | Persist timeout breadcrumb when restart readiness expires                                                                | Address | `ready_for_restart` now receives install paths and writes the recovery breadcrumb before returning `RestartDeadlineExceeded`; the timeout test reads the persisted file. |
