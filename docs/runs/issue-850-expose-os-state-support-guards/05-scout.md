...
- `docs/SPEC.md` §11.0 / Appendix K — apps must call `<Primitive>.isSupported(method)` before non-✓ methods; affected rows are `Screen.getPointerPoint`, `PowerMonitor.onPowerSourceChanged`, `SystemAppearance.getAppearance`, `SystemAppearance.onAppearanceChanged`, and `SystemAppearance.getAccentColor`.
...
- `docs/learnings/2026-05-06-linux-polish-capability-probes.md` — support probes are source-of-truth code and must be platform-scoped.
