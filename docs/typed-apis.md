...
`Desktop.rpc(AppRpc, AppRpc.toLayer(...))` returns an immutable RPC registration (a `DesktopRpcsLayer`) for the RPC group + handler pair. Compose multiple registrations with `Desktop.rpcs(...)` and pass the result as `rpcs:` to `Desktop.make`. `Desktop.manifest(App)` exposes the manifest the renderer adapter consumes.
