...
The support status is `partial`, not `supported`. Wry exposes portable `load_url`, `reload`, page-load callbacks, navigation callbacks, and script evaluation, but it does not expose a portable native back/forward/stop/loading-state API. ORIKA therefore tracks history state from host-issued commands plus Wry navigation/page-load callbacks, and uses WebView script evaluation for stop/back/forward controls.
