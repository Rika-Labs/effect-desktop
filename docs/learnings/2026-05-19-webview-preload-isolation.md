...
Preload isolation is create-time state. Wry exposes initialization-script and IPC hooks on `WebViewBuilder`, not as a portable mutation on an already-created WebView, so Effect Desktop models the policy on `WebView.create` instead of adding a misleading runtime `setPreload` route.
