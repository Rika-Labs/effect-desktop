...
The support status is `partial`. Wry compiles `open_devtools` and `close_devtools` only in debug builds unless the provider is built with devtools support, and Wry does not expose a portable debugger protocol attach API. Effect Desktop therefore keeps `attachDebugger` permission-gated and handle-validated, but returns typed unsupported with `host-debugger-protocol-unavailable`.
