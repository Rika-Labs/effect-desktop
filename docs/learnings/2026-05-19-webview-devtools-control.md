...
The support status is `partial`. Wry compiles `open_devtools` and `close_devtools` only in debug builds unless the provider is built with devtools support; on Windows, Wry can open the WebView2 devtools window but does not expose a close operation. Wry does not expose a portable debugger protocol attach API, so Effect Desktop keeps `attachDebugger` permission-gated and handle-validated, but returns typed unsupported with `host-debugger-protocol-unavailable`.
