...
- Make cleanup identity-based with both resource id and generation when a stable logical id can be reused before old resource cleanup has completed.
- After a multi-step registration returns an external handle, verify the original reservation still exists before reporting success.
