...
test("NativeParityMatrix reports declared TypeScript methods against the Rust host registry", () =>
  Effect.runPromise(
    Effect.gen(function* () {
      ...
      expect(result.rows.find((row) => row.tag === "WebView.destroy")).toMatchObject({
        hostStatus: "routed",
        support: { status: "supported" }
      })
      expect(result.rows.find((row) => row.tag === "WebView.loadRoute")).toMatchObject({
        hostStatus: "routed",
        support: { status: "supported" }
      })
      ...
      expect(result.rows.find((row) => row.tag === "WebView.reload")).toMatchObject({
        hostStatus: "routed",
        release: { status: "complete" },
        support: { status: "supported" }
      })
      expect(result.rows.find((row) => row.tag === "WebView.stop")).toMatchObject({
        hostStatus: "routed",
        release: { status: "complete" },
        support: { status: "supported" }
      })
      expect(result.rows.find((row) => row.tag === "WebView.print")).toMatchObject({
        hostStatus: "routed",
        release: { status: "complete" },
        support: { status: "supported" }
      })
      ...
      expect(result.rows.find((row) => row.tag === "WebView.goBack")).toMatchObject({
        hostStatus: "routed",
        support: {
          status: "partial",
          reason: "host-navigation-state-tracked",
      ...
      expect(result.rows.find((row) => row.tag === "WebView.postToFrame")).toMatchObject({
        hostStatus: "capability-fact",
        support: {
          status: "unsupported",
          reason: "host-frame-routing-unavailable",
      ...
      expect(result.rows.find((row) => row.tag === "WebRequest.onBeforeRequest")).toMatchObject({
        hostStatus: "capability-fact",
        support: {
          status: "unsupported",
      ...
      expect(result.rows.find((row) => row.tag === "WebRequest.removeListener")).toMatchObject({
        hostStatus: "capability-fact",
        support: {
          status: "unsupported",
          reason: "host-web-request-unavailable",
      ...
      expect(result.rows.find((row) => row.tag === "Updater.getStatus")).toMatchObject({
        hostStatus: "routed",
        support: { status: "supported" }
      })
      ...
      expect(result.rows.find((row) => row.tag === "Window.clearVibrancy")).toMatchObject({
        hostStatus: "routed",
        support: {
          status: "partial",
      ...
      expect(result.rows.find((row) => row.tag === "Window.setTitleBarTransparent")).toMatchObject({
        hostStatus: "routed",
        support: {
          status: "partial",
      ...
      expect(result.rows.find((row) => row.tag === "Tray.setMenu")).toMatchObject({
        hostStatus: "routed",
        support: {
          status: "partial",
          reason: "linux-tray-unavailable",
