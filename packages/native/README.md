...
`Window` is exposed as an Effect service. `WindowRpcs` is the matching `RpcGroup`, `makeHostWindowBridgeRpcLayer()` binds the runtime handler side to the existing host window envelopes, and `makeWindowBridgeClientLayer()` supplies the service through the typed bridge client. `WindowClient` remains the substitutable port used by tests and adapters.
