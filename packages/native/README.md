...
`Window` is exposed as the substitutable `WindowApi` Effect service. `WindowRpcs` is the full Window method descriptor with support metadata, and `Window.events.Event` is the canonical RPC stream contract. The host runtime binds handlers through canonical Effect RPC groups and bridge protocol adapters. `makeWindowBridgeClientLayer(...)` keeps the raw host `Window.Event` and `Window.subscribeEvents` protocol only as boundary compatibility while it authorizes subscriptions and reconciles `ResourceRegistry` handles.
