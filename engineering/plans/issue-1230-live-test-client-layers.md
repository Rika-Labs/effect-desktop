...
- Deterministic test layers live correctly in `@effect-desktop/test`; exporting them from `@effect-desktop/native` would invert package boundaries.
...
4. Add one shared contract suite in `@effect-desktop/test` that runs the same user-level program through direct Live, bridge Client, and Test layers for at least:
...
- Kept deterministic native test clients in `@effect-desktop/test`; #1271 already tracks generating these clients from `DesktopRpc` surfaces later.
...
- `bun run typecheck --filter=@effect-desktop/native --filter=@effect-desktop/test`
