...
The valid issue changed the final design language: `engineering/architecture/layer-first-contract.md` incorrectly referenced `Effect.Service`, while vendored Effect v4 and repo code use `Context.Service`. The fix updated the contract, `AGENTS.md`, `engineering/SPEC.md`, and `tests/layer-first-contract.test.ts`, making the rule executable instead of merely documented.
