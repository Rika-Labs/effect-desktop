...
Fourteen comments were addressed: backpressure overflow literals now match `engineering/SPEC.md`, layer handlers are typed as schema-derived functions returning `Effect`, handler maps are frozen when descriptors are built, prototype handler methods are preserved by freezing the original handler object instead of spreading it, invalid backpressure values fail as `InvalidApiContractSpec`, `cachedResultMs` is part of method metadata, idempotent methods must declare `cachedResultMs`, JavaScript/casted boolean flags are validated before registration, destructured `layer` calls preserve the contract reference, optional backpressure is guarded before freezing, schema validation accepts Effect `Schema.Class` constructor functions, handler Effects can depend on an environment, malformed permission metadata is rejected, and `Api.get` uses the repo-local `Option.none()` / `Option.some()` pattern. Two comments remained unresolved by design: restoring the constructible `class extends` form would reintroduce a synchronous boundary that cannot return duplicate/frozen/invalid failures as values, and returning a real Effect `Layer` is handler runtime wiring rather than the registry primitive in this issue.
