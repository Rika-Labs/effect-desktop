...
|                      #1882 Model PowerMonitor events as RPC streams | Closed | Removes `PowerMonitorRpcEvents`, routes direct clients through canonical `PowerMonitor.events.*` RPC streams, and keeps host event subscription translation at the bridge boundary. |
|                           #1883 Model Updater events as RPC streams | Closed | Removes `UpdaterRpcEvents`, routes direct clients through `Updater.events.PreparingRestart`, and keeps host event subscription translation at the bridge boundary.                  |
|                               #1884 Model App events as RPC streams | Closed | Removes `AppRpcEvents`, routes direct clients through canonical `App.events.on*` RPC streams, and keeps host event subscription translation at the bridge boundary.                 |
|                    #1885 Model WorkspaceIndex events as RPC streams | Closed | Removes `WorkspaceIndexRpcEvents`, routes direct clients through `WorkspaceIndex.events.Event`, and keeps host event subscription translation at the bridge boundary.               |
|                      #1886 Model EgressPolicy events as RPC streams | Closed | Removes `EgressPolicyRpcEvents`, routes direct clients through `EgressPolicy.events.DecisionRecorded`, and keeps host event subscription translation at the bridge boundary.        |
|                #1887 Model DistributionParity events as RPC streams | Closed | Removes `DistributionParityRpcEvents`, routes direct clients through `DistributionParity.events.Event`, and keeps host event subscription translation at the bridge boundary.       |
|                 #1888 Model DiagnosticsBundle events as RPC streams | Closed | Removes `DiagnosticsBundleRpcEvents`, routes direct clients through `DiagnosticsBundle.events.Event`, and keeps host event subscription translation at the bridge boundary.         |
|                #1889 Model ActivationRegistry events as RPC streams | Closed | Removes `ActivationRegistryRpcEvents`, routes direct clients through `ActivationRegistry.events.Event`, and keeps host event subscription translation at the bridge boundary.       |
...
| #1193 Generate desktop surfaces from Effect RpcGroup           | Implemented | `ScreenRpcs` is a pure `RpcGroup`, `ScreenSurface` maps generated RPC clients to the `Screen` service API, and bridge exchange use is isolated in a unary protocol adapter                           |
